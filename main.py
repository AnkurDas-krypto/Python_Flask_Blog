...
class Post(db.Model):
    ...
    date = db.Column(db.String(80), unique=False, nullable=True)
    img_file = db.Column(db.String(80), unique=False, nullable=True)
...
def edit(sno):
	if 'user' in session and session['user'] == params['user_id']:
		if request.method == 'POST':
			...
			if sno == '0':
				post = Post(title=box_title, slug=slug, tagline=tline, content=content, img_file=img_file, date=date)
				db.session.add(post)
				db.session.commit()
			else:
...
def dashboard():
	...
	if request.method == "POST":
		...
		if username == params['user_id'] and userpass == params['user_pass']:
...
def contact():
    if request.method == 'POST':
        ...
        ph_no = request.form.get('ph')
        message = request.form.get('msg')

        entry = Contact(name=name, email=email, ph_no=ph_no, date=datetime.now(), message=message)
